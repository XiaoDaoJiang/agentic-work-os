...
fn parse_batch(input: &str) -> Result<StageBRun, RunnerError> {
    ...
    for (index, line) in input.lines().enumerate() {
        ...
        let request = serde_json::from_str::<Request>(line)
            .map_err(|error| protocol_error(format!("line {} is invalid JSON: {error}", index + 1)))?;
...
fn wait_for_exit(
    ...
    run_id: &str,
    timeout: Duration,
) -> Result<(Option<i32>, bool, JoinState), RunnerError> {
    let started = Instant::now();
    let mut exit_status: Option<ExitStatus> = None;
    let mut timed_out = false;
    let mut streams = JoinState::default();
    ...
    while exit_status.is_none() || !streams.complete() {
            ...
            Err(RecvTimeoutError::Disconnected) => {
                if !streams.complete() {
                    return Err(RunnerError::Execution(
                        "stream readers disconnected before both streams drained".to_string(),
                    ));
                }
            }
        }
        ...
        if exit_status.is_none() {
            ...
            } else if let Some(status) = child.try_wait().map_err(|error| {
...
fn execute(run: StageBRun) -> Result<(), RunnerError> {
    ...
    let timeout = Duration::from_millis(run.start.timeout_ms);
    ...
    writer.emit(
        &run_id,
        "stdout.drained",
        json!({"final_sequence": writer.stream_sequence(StreamKind::Stdout)}),
    )?;
    writer.emit(
        &run_id,
        "stderr.drained",
        json!({"final_sequence": writer.stream_sequence(StreamKind::Stderr)}),
    )?;
    writer.emit(
        &run_id,
        "boundary.snapshot",
...
fn decode_base64(value: &str) -> Result<Vec<u8>, RunnerError> {
    ...
    for (index, chunk) in encoded.chunks_exact(4).enumerate() {
        ...
        let first = base64_value(chunk[0]).ok_or_else(|| protocol_error("bytes_base64 character is invalid"))?;
        let second = base64_value(chunk[1]).ok_or_else(|| protocol_error("bytes_base64 character is invalid"))?;
        let third = if chunk[2] == b'=' { 0 } else { base64_value(chunk[2]).ok_or_else(|| protocol_error("bytes_base64 character is invalid"))? };
        let fourth = if chunk[3] == b'=' { 0 } else { base64_value(chunk[3]).ok_or_else(|| protocol_error("bytes_base64 character is invalid"))? };
...
fn read_stdin() -> Result<String, RunnerError> {
...
fn main() {
    let result = read_stdin().and_then(|input| parse_batch(&input)).and_then(execute);
